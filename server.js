...
// POST /api/cues/reimport-text  { projectId, fileId, languageId, jwtToken }
// Pulls fresh TEXT from Crowdin's current source strings/translations for
// this language only - timing and cue count/order (deletes, clones/splits)
// are left completely untouched, per instruction.
//
// Cues that were cloned/split share ONE underlying Crowdin string with
// their sibling(s) (that's how splitting works today - see
// lib/languageCues.js / the /api/cues/clone handler above). Since a source
// string only has one translation, reimporting text for a split cue has no
// way to know which half should receive it without silently overwriting the
// linguist's manual split - so any cue whose sourceStringId is shared with
// another cue in the same list is left untouched. Everything else (plain
// cues, and cues that were only timing-adjusted or survived a delete) maps
// 1:1 to a real Crowdin string, so refreshing their text is unambiguous.
app.post("/api/cues/reimport-text", requireJwt, async (req, res) => {
  try {
    const { projectId, fileId, languageId } = req.body;
    const domain = req.crowdinContext.domain;
    const accessToken = await auth.getAccessToken(domain);

    const [file, stringRecords, translations] = await Promise.all([
      crowdin.getFile(accessToken, domain, projectId, fileId),
      crowdin.listSourceStrings(accessToken, domain, projectId, fileId),
      crowdin.listLanguageTranslations(accessToken, domain, projectId, languageId, fileId),
    ]);
    const liveTextFor = (sourceStringId) => {
      const translated = translations.get(sourceStringId);
      if (translated !== undefined) return translated;
      const source = stringRecords.find((s) => s.id === sourceStringId);
      return source ? source.text : undefined; // fall back to source text if untranslated
    };
    ...
    if (customCues) {
      ...
      customCues.forEach((c) => {
        shareCount[c.sourceStringId] = (shareCount[c.sourceStringId] || 0) + 1;
      });
      const next = customCues.map((c) => {
        if (shareCount[c.sourceStringId] > 1) return c; // part of a split - leave as-is
        const fresh = liveTextFor(c.sourceStringId);
        return fresh === undefined ? c : { ...c, text: fresh };
      });
      await languageCues.writeForLanguage(axios, accessToken, domain, projectId, file, languageId, next);
      return res.json({ cues: next.map((c) => ({ ...c, isOverridden: true, isCustom: true })) });
    }

    // Not yet customized: strip just the `text` key from every string's
    // per-language override (any custom timing on that same string is left
    // alone), so the next read falls back to Crowdin's live translation.
    await Promise.all(
      stringRecords.map((s) => timingField.clearTextOverride(axios, accessToken, domain, projectId, s, languageId))
    );
    const cues = await buildCues(accessToken, domain, projectId, fileId, languageId);
    res.json({ cues });
  } catch (err) {
