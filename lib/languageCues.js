 ...
 *             "finished": true, "finishedAt": "2026-07-29T...", "finishedBy": "Maria" },
 *     "es": { "customized": false }  // or simply absent - still derived live, as before
 *   }
 * The "finished"/"finishedAt"/"finishedBy" keys are a separate, later
 * addition (see readFinishStatus/writeFinishStatus below) - a linguist's
 * "I'm done with this file+language" marker, orthogonal to "customized".
 * They ride in the same per-language entry purely so the finished flag
 * benefits from the chunked-field overflow handling below without
 * duplicating that machinery for a second field.
 *
...
/**
 * "Finished" status - a linguist-facing marker, separate from the
 * "customized" cue-ownership flag above, for "I'm done with this
 * file+language, please look at it." Lives in the SAME per-language blob
 * (and rides the same chunked-field storage) rather than a new field,
 * since it's small and there's no reason to duplicate the ensure/read/
 * write/chunk machinery for one more boolean-ish value.
 *
 * finishedAt/finishedBy are deliberately left in place when a linguist
 * "unfinishes" (only `finished` flips back to false) - that's the last
 * time this file+language WAS marked finished, useful context if someone
 * looks at it later, and gets overwritten the next time Finish is clicked.
 */
function readFinishStatus(fileRecord, languageId) {
  const entry = readAll(fileRecord)[languageId] || {};
  return {
    finished: Boolean(entry.finished),
    finishedAt: entry.finishedAt || null,
    finishedBy: entry.finishedBy || null,
  };
}
 ...
 *
 * `partialUpdate` is shallow-merged into whatever entry already exists for
 * this language (so writing a finished-status update doesn't clobber an
 * existing customized cue list, and vice versa).
 */
async function writeEntryUpdate(axios, accessToken, domain, projectId, fileRecord, languageId, partialUpdate) {
  const api = client(axios, accessToken, domain);
  const all = readAll(fileRecord);
  all[languageId] = { ...(all[languageId] || {}), ...partialUpdate };
...
/** Persist the full cue list for one language, marking it customized - see
 * writeEntryUpdate() above for the actual chunked-write mechanics. */
function writeForLanguage(axios, accessToken, domain, projectId, fileRecord, languageId, cues) {
  return writeEntryUpdate(axios, accessToken, domain, projectId, fileRecord, languageId, { customized: true, cues });
}

/** Mark a (file, language) finished (with who/when) or unfinished. Passing
 * `{ finished: true, finishedBy }` sets finishedAt to now server-side;
 * passing `{ finished: false }` just flips the flag back, leaving the last
 * finishedAt/finishedBy in place (see readFinishStatus's doc comment). */
function writeFinishStatus(axios, accessToken, domain, projectId, fileRecord, languageId, { finished, finishedBy }) {
  const update = finished
    ? { finished: true, finishedAt: new Date().toISOString(), finishedBy: finishedBy || null }
    : { finished: false };
  return writeEntryUpdate(axios, accessToken, domain, projectId, fileRecord, languageId, update);
}

module.exports = {
  FIELD_NAME,
  FIELD_SLUG,
  ensureLanguageCuesField,
  readForLanguage,
  writeForLanguage,
  readFinishStatus,
  writeFinishStatus,
