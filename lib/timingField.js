...
/**
 * Reimport support: strip just the `text` key from one language's override
 * entry, keeping any custom `startMs`/`endMs` on that entry untouched. If
 * removing `text` leaves nothing else in the entry, the whole per-language
 * key is dropped (rather than left as a stray empty object, which would
 * otherwise still read back as "isOverridden: true" with nothing actually
 * overridden).
 *
 * No-ops (returns null without writing) if this string has no text override
 * for this language, so callers can safely call this for every string in a
 * file without generating a wave of pointless PATCH requests.
 */
async function clearTextOverride(axios, accessToken, domain, projectId, stringRecord, languageId) {
  const overrides = readOverrides(stringRecord);
  const existing = overrides[languageId];
  if (!existing || existing.text === undefined) return null;

  const { text, ...rest } = existing;
  if (Object.keys(rest).length > 0) {
    overrides[languageId] = rest;
  } else {
...
module.exports = { FIELD_NAME, FIELD_SLUG, ensureTimingField, readOverrides, writeOverride, clearTextOverride };
