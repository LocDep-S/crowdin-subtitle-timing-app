...
(function () {
  ...
  function mountPlayer(videoUrl) {
    ...
    loadYouTubeApiThen(() => {
      state.player = new YT.Player("player", {
        ...
        //
        // fs: 0 removes YouTube's own fullscreen button. That button
        // fullscreens the <iframe> itself, and #subtitle-overlay lives
        // outside the iframe (a cross-origin document we can't inject into)
        // - so the native fullscreen control would silently drop the
        // overlay, leaving whatever's baked into the video/YouTube's own UI
        // as the only thing visible. #fullscreen-btn below fullscreens
        // #player-wrap instead, keeping video + overlay together.
  ...
  //
  // Renders <i>/<b>/<u> as real italics/bold/underline (common SRT emphasis
  // tags - on-screen text, whispers, song lyrics), since a linguist should
  // see the overlay the same way it'll actually look once exported. This is
  // deliberately NOT a general HTML sanitizer or raw innerHTML of the cue
  // text: that text comes from translators/Crowdin, not from us, so treating
  // arbitrary <tags> as live HTML would let subtitle content inject markup
  // into this page. safeSubtitleHtml below escapes everything first, then
  // re-expands ONLY the exact escaped forms of these three bare tags - any
  // other tag name, or a tag with attributes (e.g. <i onclick=..>), stays
  // escaped and shows as literal text instead of running.
  ...
  function startTimeSync() {
    setInterval(() => {
      ...
      // Same mm:ss.mss format as the start/end inputs (msToInputValue,
      // defined below in "Cue list rendering") - a linguist can pause on the
      // exact frame a line starts/ends and type this value straight into the
      // corresponding field instead of eyeballing it. getCurrentTime() keeps
      // returning the right value while paused too, not just during
      // playback, so this stays accurate either way.
      els.timeReadout.textContent = msToInputValue(currentMs);
    }, 250);
  }

  // ---- Fullscreen (custom, see the `fs: 0` comment in mountPlayer above) --
  ...
  // Character-count column to the right of each cue's text - lets a linguist
  // glance at line length / total length without having to eyeball it or
  // paste into an external counter. Per-line counts only show up once a cue
  // actually has more than one line (a single-line cue's line count and
  // total are the same number, so showing both would just be noise). Counts
  // are raw character length of each line, including any <i>/<b>/<u> markup
  // characters typed in - this deliberately mirrors what's actually in the
  // field/exported .srt, not the rendered-on-screen length once tags are
  // stripped, since that's a much bigger judgement call (which tags "don't
  // count") this feature isn't trying to make.
  function cueCharCountHtml(text) {
    const lines = text.split("\n");
    const totalChars = lines.reduce((sum, line) => sum + line.length, 0);
    const perLineHtml =
      lines.length > 1
        ? lines.map((line, i) => `<div class="char-line">L${i + 1}: ${line.length}</div>`).join("")
        : "";
    return `${perLineHtml}<div class="char-total">${totalChars} ch</div>`;
  }
  ...
  function renderCues() {
    ...
    state.cues.forEach((cue) => {
      ...
      li.innerHTML = `
        ...
        <div class="char-info" title="Characters per line, and total characters for this cue">
          ${cueCharCountHtml(cue.text)}
        </div>
      `;

      const textEl = li.querySelector(".text");
      const charInfoEl = li.querySelector(".char-info");
      textEl.addEventListener("click", () => seekTo(cue.startMs));
      textEl.addEventListener("change", () => saveText(cue.id, textEl.value));
      // Updates live on every keystroke (not just on save/blur) - the whole
      // point is a glanceable check WHILE editing, before committing a line
      // that's too long.
      textEl.addEventListener("input", () => {
        charInfoEl.innerHTML = cueCharCountHtml(textEl.value);
      });
  ...
  // ---- Reimport text ----------------------------------------------------
  // Pulls fresh text from Crowdin's current source/translation for this
  // language, WITHOUT touching timing or cue count/order - the fix for
  // "I edited text on the right by mistake and now Crowdin's own updates
  // don't show up here". Cues that were cloned/split are left untouched
  // (see server.js) since a split shares one Crowdin string with its
  // sibling - there's no way to tell which half should get the fresh text
  // without erasing the manual split.

  els.reimportBtn.addEventListener("click", async () => {
    if (
      !confirm(
        "Reimport text from Crowdin for this language?\n\nThis overwrites any text edited here with Crowdin's current translation. Timing, and any cloned/split cues, are left exactly as they are (split cues are skipped, since they share one Crowdin string)."
      )
    ) {
      return;
    }
    setStatus("Reimporting text…");
    try {
      const result = await apiPost("/api/cues/reimport-text", {
        projectId: state.projectId,
        fileId: state.fileId,
        languageId: state.languageId,
      });
  ...
  async function init() {
    ...
    // Crowdin's own Editor lets a linguist browse strings across every file
    // in the project at once ("All files" in the file switcher) - in that
    // mode context.editor.file comes through as the literal string "all"
    // instead of a real numeric file id. There's no single file for Video &
    // Timing to show in that view, and passing "all" straight through used
    // to hit Crowdin's API as GET /projects/{id}/files/all, which 404s since
    // "all" isn't a real file - surfacing as a bare "Request failed with
    // status code 404" with no indication of what to actually do about it.
    if (state.fileId === "all") {
      setStatus('Video & Timing works with one file at a time - pick a specific file from the file switcher above the editor (not "All files").');
      return;
    }
